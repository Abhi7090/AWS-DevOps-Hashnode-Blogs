...
* 1. Clone the repo : [https://github.com/LondheShubham153/two-tier-flask-app.git](https://github.com/your-username/your-repo-name.git)
    ...
    5. Now add network to two images due to interaction between two images
        ...
        docker run -d -p 3306:3306 -e MYSQL\_ROOT\_PASSWORD=test@123 -e MYSQL\_DATABASE=test\_db -e MYSQL\_USER=admin -e MYSQL\_PASSWORD=admin --name --network two-tier-app-nw mysql-app mysql:latest
        
        docker run -d -p 5000:5000 -e MYSQL\_HOST=mysql-app -e MYSQL-USER=admin -e MYSQL\_PASSWORD=admin -e MYSQL\_DB=test\_db --name flaskapp --network two-tier-app-nw flask-app:latest
